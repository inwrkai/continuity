...
A draft record is a candidate knowledge record representing one real-world item (task, appointment, decision, etc.).
...
- Prefer `updates_record_id` when the draft clearly refers to the same real-world item as a prior record (match on title/description/fields, not just object type)
...
Produce a revised JSON array of drafts. Edit the draft set directly — do not emit a separate feedback-action vocabulary.

### What to fix

1. **Merge** — two drafts refer to the same real-world item → combine into one (merge fields and citations; keep higher confidence)
...
4. **Edit** — title, description, or fields miss key transcript context → revise
5. **Relink** — `updates_record_id` is wrong or missing when clearly an update → correct or set to `null`
...
Return the full revised drafts array (same schema as Stage 1). This is the final set used for writing.
